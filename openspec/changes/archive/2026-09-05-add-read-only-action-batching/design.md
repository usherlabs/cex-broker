...
- Retain durable implementation and verification evidence in the pull request or release record and provide a downstream Maker adoption handoff after the broker implementation is verified.
...
1. record the final contracts, verification evidence, live-smoke status, and revision in the pull request or release record;
2. provide a secret-free adoption handoff to the downstream Maker maintainers through their project documentation or an agreed delivery channel;
...
The pull request or release record retains durable delivery evidence; the handoff supports the downstream Maker implementation without requiring a particular workstation or agent workflow.
...
- **External handoffs may become unavailable** → Retain the broker revision and essential adoption summary in the pull request or release record so delivery evidence does not depend on a workstation-local file.

## Migration Plan

1. Record the approved plan, assign one implementation owner, and note the approved hard cuts and Maker-handoff boundary in the pull request or release record.
...
7. Finalize the delivery evidence and provide the Maker adoption handoff; downstream Maker work updates its canonical contract, generated decoders, batch client, and materializer before coordinated deployment.
...
None. The approved design uses registry-derived read-only batching, preserves the protobuf request/response message shapes, hard-cuts the three evidence boundaries, keeps child decoding consumer-owned, and delivers Maker adoption through durable delivery evidence plus a downstream adoption handoff.
