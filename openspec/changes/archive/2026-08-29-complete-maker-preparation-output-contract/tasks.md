...
- [x] 8.1 Complete every reopened pre-freeze task in sections 9 and 10, and have Maker prove deterministic clean-extraction invocation of both role-neutral preparation-library operations plus ownership of the cross-stage and cross-pair state machine without live credentials. Do not reserve a version while this gate is open.
...
- [ ] 8.3 From a clean checkout of the frozen commit, have Maker select the largest recent common interval from 30 down to no fewer than 20 complete UTC days and invoke pair-local CEX source-tape preparation for OKX Spot ARB-USDC and ARB-USDT over that exact interval; retain separate CEX qualification records, inventories, receipts, selections, export results, and durable pair outcomes without Candidate-role or aggregate-pair interpretation in CEX. Treat `[2026-07-26T00:00:00Z, 2026-08-19T00:00:00Z)` only as historical evidence unless independently selected again.
...
- [ ] 8.6 Submit each Maker-produced clock through the pinned `runMarketDataRequiredClockQualification` package operation and produce an exhaustive exact three-way source disposition without teaching CEX its Candidate role, scheduler, DEX inputs, or target-to-invocation semantics.
...
- [x] 9.1 Make forensic-ledger validation contextual on the authoritative required-clock document: prove exact target membership, ordering, identity/time equality, count reconciliation, and retained-record interval causality; reject unknown, duplicate, omitted, altered, or unsupported disposition references.
...
- [x] 9.3 Convert forensic-ledger v1 and qualification-record v1 into closed `required_clock_qualification`/`source_tape` operation unions; replace CEX `derivation_eligible` and Candidate-named predicates with role-neutral, operation-applicable `source_partition_complete`, `source_event_enumeration_eligible`, and `source_tape_eligible`, while keeping clock `qualified` strictly all-fresh with `source_reconstruction_accepted`.
- [x] 9.4 Bind the complete expected and observed provider-object/selected-interval inventory so source-event enumeration eligibility is positively proved rather than inferred from missing defects.
- [x] 9.5 Refactor the policy-neutral OKX top-100 state/freshness-change runner and exhaustive required-clock qualifier from repository-only tooling into exported functions `runMarketDataSourceTape` (`market-data-source-tape/v1`) and `runMarketDataRequiredClockQualification` (`market-data-required-clock-qualification/v1`) on `@usherlabs/cex-broker/market-data-preparation`; implement their closed inputs, host-neutral normalized invocation hashes, and pair-local terminal evidence without a third executable, thirteenth schema, Maker policy filtering, or broker/server import.
- [x] 9.6 Rename Candidate-specific tape construction/capability identities to role-neutral source-tape identities and regenerate dependent policies, fixtures, and product-pin v2 so one preparation-library identity binds the exported subpath/runtime/declaration hashes and exact two-symbol operation manifest separately from exactly two executable identities; retain exactly twelve package schemas by reusing the levels and depth-summary projections only after semantic validation.
- [x] 9.7 Remove the Maker derivation-descriptor schema copy and semantic validator from CEX qualification authority; add architecture tests proving CEX does not inspect Maker scheduler/configuration, DEX inputs, invocation mappings, blocked outcomes, or Candidate roles.
- [x] 9.8 Keep only the generic CEX 100,000-target request ceiling and exact no-deduplication behavior; remove Maker-invocation and Candidate-capacity preflight ownership from CEX and add deterministic clean-extraction fixtures proving Maker can call both closed APIs without a sibling checkout, repository script, credential, or host-path-dependent identity.
- [x] 9.9 Add adversarial tests for exact clock membership and record causality, operation-union exclusivity, rejected clock fields on source-tape input, required clock on qualification input, role-neutral identical-clock behavior, source-partition and enumeration predicates, positive inventory, source-tape completeness, consumer-visible initializer identity, terminal qualification records for both operations, absent Maker semantics, relocated attempt roots, and the 100,000-target whole-ledger bound.
- [x] 9.10 Re-run unit, type, build, generated-identity, clean-extraction library/import, product-pin, package, strict OpenSpec, architecture, and secret-reflection validation; correct invalidated historical evidence claims and record remaining credential/source/Maker-owned operational work without marking it complete.
...
- [x] 10.8 Re-run targeted adversarial RED/GREEN tests, repository unit suite, type/build, changed-file formatting, strict OpenSpec validation, package audit, ClickHouse smoke, and secret-reflection checks without licensed live qualification or publication.
