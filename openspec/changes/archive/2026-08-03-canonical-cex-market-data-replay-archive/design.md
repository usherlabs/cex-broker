...
- Define or implement Maker `hb_runtime` delivery durability, `strategy_data.*` schema-version validation, spool recovery, or retry ownership. The dedicated `maker-archive-forwarder-conformance` change owns that cross-service contract.
...
7. Treat the documented production observation window for feed health, queue behavior, checksum verification, and Maker consumption as an external deployment cutover gate owned by FIET-937. It is not an OpenSpec implementation-completion task and repository tests do not claim it has occurred.
