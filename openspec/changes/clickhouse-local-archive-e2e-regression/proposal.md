...
Archive tests currently exercise subscription capture, queue durability, HTTP forwarding, and ClickHouse schemas in separate layers, while the ClickHouse-backed suite silently returns when no server is available. The canonical archive work expands the storage contract, so a required, deterministic regression gate is needed to prove the complete subscription-to-storage lifecycle without losing any pre-canonical archive behavior.
...
- Require `CEX_BROKER_ARCHIVE_SOURCE=broker_write` plus `CEX_BROKER_MARKET_ARCHIVE_WRITE_MODE=dual` to preserve exact legacy rows and values for ORDERBOOK, TICKER, TRADES, and OHLCV while proving a linked raw capture for every feed.
- Require `CEX_BROKER_ARCHIVE_SOURCE=broker_read` plus `CEX_BROKER_MARKET_ARCHIVE_WRITE_MODE=canonical` to prove per-feed raw-to-normalized linkage, capture and provider provenance, stored checksums, canonical views, and order-book-specific duplicate and checksum-conflict behavior. Pure `legacy` mode is outside this E2E matrix.
...
- Add a required `test:e2e:archive` CI gate and a separate scheduled/manual, non-merge-gating `test:smoke:archive` workflow limited to public market-data subscriptions.
...
- Package test commands and CI configuration, plus a scheduled/manual live-CEX smoke workflow and its read-only secret-backed configuration.
...
- Applying the tasks requires the canonical archive implementation to be integrated first; authoring and validating this separate OpenSpec does not modify that prerequisite change.
