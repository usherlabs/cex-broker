...
- [x] 1.1 Confirm the integrated runtime satisfies the final `canonical-cex-market-data-replay-archive` artifacts, record its commit, and verify this change does not edit those canonical OpenSpec files.
...
- [x] 3.9 Run the verified E2E command before completing the harness, capture behavioral RED evidence as an apply-phase acceptance artifact, and reject missing imports, missing fixtures, unavailable binaries, schema or archive-configuration failures, skips, or no-test discovery as acceptable RED proof.
...
- [x] 5.1 Post fixture-driven batches through the real local `/archive` endpoint for every baseline table not produced by the public-feed lifecycle.
...
- [x] 6.5 With `CEX_BROKER_ARCHIVE_SOURCE=broker_write` and `CEX_BROKER_MARKET_ARCHIVE_WRITE_MODE=dual`, prove exact ORDERBOOK, TICKER, TRADES, and OHLCV baseline projections and cardinalities, including fixed times, identifiers, arrays, and replacement/closed-candle semantics.
- [x] 6.6 In dual mode, query the closed canonical inventory and prove at least one `market_data.cex_stream_events` raw row per feed links to its normalized output without changing or duplicating any legacy result; fail every unexpected destination or unclassified row.
- [x] 6.7 With `CEX_BROKER_ARCHIVE_SOURCE=broker_read` and `CEX_BROKER_MARKET_ARCHIVE_WRITE_MODE=canonical`, prove every feed has at least one `market_data.cex_stream_events` raw capture linked by capture bundle and raw-capture id to its named normalized output, with provider, exchange/pair/feed, source mode, schema version, and source/received time provenance.
- [x] 6.8 Recompute raw and normalized checksums from queried stored projections for all four feeds and compare them with stored checksum values and algorithm versions.
- [x] 6.9 Prove `market_data.cex_order_book_levels_canonical`, `market_data.cex_order_book_depth_summary_canonical`, and `market_data.cex_ohlcv_closed` return the expected conflict-free rows; prove both named conflict views are empty and canonical mode writes no legacy order-book or candle rows for its lifecycle identity.
...
- [x] 8.6 Document local E2E prerequisites, deterministic archive configuration, verified binary override/cache behavior, the pinned baseline-regeneration host recipe, fixture provenance, failure diagnostics, and cleanup expectations.

## 9. Live CEX Smoke Workflow

- [x] 9.1 Add `test:smoke:archive` with provisioned-only read-only credential configuration, a unique capture bundle, `CEX_BROKER_ARCHIVE_SOURCE=broker_read`, `CEX_BROKER_MARKET_ARCHIVE_WRITE_MODE=canonical`, and no request-supplied credential override.
...
- [x] 9.5 Add a scheduled/manual-only workflow with secret-backed read-only credentials, no pull-request trigger, and no merge-required status.
...
- [x] 9.7 Document operator attestation of read-only exchange scope and verify the smoke never probes permissions with an order, cancellation, transfer, deposit, withdrawal, or asset movement.

## 10. Final Verification

- [x] 10.1 Re-run baseline regeneration twice and verify the committed fixture and provenance manifest remain unchanged.
- [x] 10.2 Run `test:e2e:archive` from a clean shallow/offline-compatible checkout using only the committed fixture and verified binary cache/override.
- [x] 10.3 Run the normal repository test command and the retained ClickHouse-server integration suite with a provisioned server when available. No provisioned server was available in this session; the retained integration test command completed while its transport probe reported `ECONNREFUSED`.
- [x] 10.4 Run repository lint/check, server line-budget, type, and build commands required by CI and resolve every failure.
...
- [x] 10.6 Run `openspec validate clickhouse-local-archive-e2e-regression --strict` and resolve every validation error.
- [x] 10.7 Review the final diff to prove the canonical OpenSpec artifacts are unchanged, fixture expectations were not weakened after RED, no E2E skip path exists, and no production protobuf/RPC/public API changed.
- [x] 10.8 Record final GREEN evidence for the dedicated E2E command, normal tests, build/type checks, strict OpenSpec validation, and the non-gating smoke result.
