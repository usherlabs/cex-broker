...
- [x] 2.5 Add credential-source policy configuration for provisioned-only versus approved request-metadata credentials and explicit `public` versus `read_only_key` provisioned profiles.
- [x] 2.6 Reject any request-supplied exchange credential metadata with gRPC `PERMISSION_DENIED` before broker construction under provisioned-only policy, emit a bounded rejection metric, and test that credential values are never logged.
- [x] 2.7 Enforce explicit public construction and fail missing/invalid `read_only_key` profiles without silently falling back to public access.
- [x] 2.8 Add a regression test proving TEE/non-TEE, archive source, and credential policy do not change registered RPCs or action handlers.
...
- [x] 3.6 Add tests proving capture source, deployment, bundle, and credential provenance cannot be overridden by provider payload or untrusted request fields.
...
- [x] 5.2 Replace canonical candle output with a `cex_ohlcv` row builder while retaining a migration-only legacy candle builder.
...
- [x] 8.1 Add bounded deployment phases for dual-writing `orderbook_snapshots` to canonical order-book tables and `candles` to `cex_ohlcv`.
- [x] 8.2 Add parity queries and metrics comparing legacy snapshots/candles with canonical summaries, levels, and OHLCV rows.
- [x] 8.3 Add idempotent backfill paths from `orderbook_snapshots` and `candles` that stamp legacy source mode and incomplete provenance while leaving unavailable capture bundles/raw identities/raw checksums null.
...
- [x] 8.6 Document and test cutover and rollback procedures that never drop legacy or canonical data.
...
- [x] 9.4 Export canonical order-book rows to Maker-compatible parquet fixtures and validate capture-core field and checksum parity.
...
- [x] 9.6 Add non-destructive deployment checks for explicit non-TEE `public` and `read_only_key` profiles, provisioned-only metadata rejection, and exchange-side read-only attestation without placing orders or moving assets.
...
- [x] 10.1 Document archive source, capture bundle, explicit credential profiles, metadata rejection, external-fallback scope, feed configuration, checksum version, canonical/conflict views, and order-book/OHLCV migration settings in the broker README and service documentation.
- [x] 10.2 Document that TEE and non-TEE use the same full RPC surface and that non-TEE read-only posture depends on provisioned exchange permissions and credential-source policy.
