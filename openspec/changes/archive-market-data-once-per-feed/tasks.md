...
- [ ] 4.4 Add a band-coverage verifier that requires farthest retained bid/ask prices to cross every Maker boundary or explicit book exhaustion; include a negative case where P retained levels still stop inside the required band and emit venue/profile/band/side diagnostics.
- [ ] 4.5 Configure the replay gate with policy depth P and archive depth at least P, rehydrate L2 inputs from ClickHouse, and compare live versus archived mid, band depths, limiting side, and coverage verdict; include a negative 25-level archive case that cannot reproduce a deeper policy.
- [ ] 4.6 Drive live reference snapshots and independently rehydrated conservative/coalesced archive inputs through FIET Maker's shared position-policy/hedge-envelope implementation; compare live-versus-rehydrated and cross-composition band depths, limiting side, envelope liquidity cap, selected width ticks, and rebalance decision at every evaluation.
- [ ] 4.7 Require independent Binance and MEXC resolver cases to pass logical/archive equality, band coverage, replay sufficiency, and downstream position-policy equivalence before their coalescing profiles are enabled by default.
- [ ] 4.8 Extend the four-feed archive lifecycle with overlapping compatible ORDERBOOK collector/client depths and a zero-bootstrap-first OHLCV client followed by the positive-bootstrap collector, proving one physical worker/tracker/archive bootstrap and correct live delivery.
- [ ] 4.9 Update production-compatible Maker sidecar scripts/verifier so the real Maker and collector identify the shared Binance/MEXC acquisition profile, retain omitted-versus-explicit response contracts, prove two logical deliveries with one physical watch/archive capture, and report immediate-hedgeability policy equivalence; retain independent reference-depth action evidence.
- [ ] 4.10 Update `SERVICES_ARCHITECTURE.md` to describe collector coverage/liveness, L2 immediate-hedgeability evidence, policy-versus-archive depth alignment, venue-profile resolution, verified Binance/MEXC coalescing, conservative unknown-venue isolation, and multiple logical clients sharing compatible broker-owned physical feeds.
...
- [ ] 5.3 Run the integrated archive lifecycle, Binance and MEXC conservative-versus-coalesced collector gates, L2 band-coverage and 25-level replay-negative cases, and production-compatible sidecar conformance path, retaining evidence for logical/archive/policy equivalence, reduced physical work, successful strategy spool drainage, and canonical ClickHouse output.
