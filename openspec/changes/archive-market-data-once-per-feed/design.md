...
- Gate Binance/MEXC coalescing on collector-observed equivalence with the conservative depth-isolated model over a bounded deterministic observation window.
...
3. Add Binance/MEXC resolvers behind conservative fallback, update controlled-exchange probes, and make the collector equivalence comparison a required E2E gate before enabling coalescing.
...
None. The acquisition-profile key, initial Binance/MEXC resolver scope, conservative fallback, broker precedence, buffer/error contract, late-join policy, failure ownership, and retryable first-positive OHLCV bootstrap semantics are fixed by this change.
