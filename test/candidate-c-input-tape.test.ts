...
import {
	...
	CANDIDATE_C_INPUT_TAPE_CAPABILITY_ID,
	CANDIDATE_C_INPUT_TAPE_CONSTRUCTION_MODE,
	CANDIDATE_C_INPUT_TAPE_MAX_BATCH_BYTES,
	CANDIDATE_C_INPUT_TAPE_MAX_BATCH_ROWS,
	CANDIDATE_C_INPUT_TAPE_MAX_IN_FLIGHT,
	CANDIDATE_C_INPUT_TAPE_MAX_STATES_PER_YIELD,
	CANDIDATE_C_INPUT_TAPE_PROJECTION_PINS,
	CANDIDATE_C_INPUT_TAPE_SANDBOX_TARGET,
	createCandidateCInputTapeArchiveSink,
	evaluateCandidateCInputTapeEligibility,
	normalizeCandidateCInputTapeStates,
...
describe("Candidate C policy-neutral input tape", () => {
	test("pins bounded sandbox submission and preserves authorization-class semantics", () => {
		expect(CANDIDATE_C_INPUT_TAPE_MAX_STATES_PER_YIELD).toBe(4);
		expect(CANDIDATE_C_INPUT_TAPE_MAX_BATCH_ROWS).toBe(1_000);
		expect(CANDIDATE_C_INPUT_TAPE_MAX_BATCH_BYTES).toBe(5_242_880);
		expect(CANDIDATE_C_INPUT_TAPE_MAX_IN_FLIGHT).toBe(1);
		expect(CANDIDATE_C_INPUT_TAPE_SANDBOX_TARGET).toEqual({
			environment: "sandbox",
		...
		expect(() =>
			assertCandidateCInputTapeSandboxAuthorization({
				...
				requestTarget: CANDIDATE_C_INPUT_TAPE_SANDBOX_TARGET,
				preflight: {
		...
		).toThrow("candidate_c_input_tape_sandbox_target_mismatch");
	});
	...
	test("retains intervening OKX state changes that Candidate A sampling omits", () => {
		const request = validBackfillRequest({
			providerPolicy: {
				provider: "cryptohftdata",
				allowedAdapterVersions: ["cryptohftdata-orderbook/v2"],
		...
		const canonicalRows = normalizeCandidateCInputTapeStates({
			tape,
			capture_bundle_id: "b".repeat(64),
		...
		).toEqual(new Set([CANDIDATE_C_INPUT_TAPE_CONSTRUCTION_MODE]));
	});
	...
	test("requires positive complete inventory and both bootstrap gates", () => {
		const inventory = {
		...
		expect(
			evaluateCandidateCInputTapeEligibility({
				bootstrap_qualified: true,
				source_enumeration_eligible: true,
				provider_object_inventory: inventory,
				tape_complete: true,
				capability_id: CANDIDATE_C_INPUT_TAPE_CAPABILITY_ID,
				capability_sha256: CANDIDATE_C_INPUT_TAPE_CAPABILITY.policy_sha256,
				construction_mode: CANDIDATE_C_INPUT_TAPE_CONSTRUCTION_MODE,
